...
An executable visual must either run the same authoritative executable model as
the lesson or prove a deliberately small duplicated calculation against
independently specified literal parity vectors. Exercise those vectors through
both implementations and fail verification when either disagrees. A browser
self-check may expose the result for inspection, but it is engineering evidence,
not evidence that a learner understood the model.
